...
const long unsigned int JOINT_LIMIT = 128;
...
void CreateModel(char* file, char* animation);
void DrawModel(void);
...
void Initialize(int argc, char* argv[]) {
  if (argc < 2) {
    fprintf(stderr, "Usage: %s filename.egg [animation.egg]\n", argv[0]);
    return;
  }
  ...
  if (argc >= 3) {
    CreateModel(argv[1], argv[2]);
  } else {
    CreateModel(argv[1], NULL);
  }
}
...
void game_loop(void) {
  ...
  while (playing && glfwGetWindowParam(GLFW_OPENED)) {
    ...
    model->jointTree.runJoint(mat, dt);
    collectJoints();
...
void CreateModel(char* file, char* animation) {
  ...
  glBindAttribLocation(ShaderIds[0], 2, "in_joints");
  glBindAttribLocation(ShaderIds[0], 3, "in_weight");
  ...
  JointsUniformLocation = glGetUniformLocation(ShaderIds[0], "Joints");
  OnGLError("ERROR: Could not get Joints uniform locations");
...
void DrawModel(void) {
  ...
  glUniformMatrix4fv(JointsUniformLocation, JOINT_LIMIT, GL_FALSE,
      (const GLfloat*) Joints);
