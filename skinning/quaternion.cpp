...
Matrix Quaternion::toMatrix() const {
  ...
  ret.data[0*width+0] = 1 - 2 * (yy + yz);
  ret.data[1*width+0] = 2 * (xy - zw);
  ret.data[2*width+0] = 2 * (xz + yw);

  ret.data[0*width+1] = 2 * (xy + zw);
  ret.data[1*width+1] = 1 - 2 * (xy + yz);
  ret.data[2*width+1] = 2 * (yz - xw);
  
  ret.data[0*width+2] = 2 * (xz - yw);
  ret.data[1*width+2] = 2 * (yz + xw);
  ret.data[2*width+2] = 1 - 2 * (xy + yy);
